...
export default function ImportData() {
  ...
  // NEW: State for handling encrypted PDFs
  ...
  const executeClearGrid = () => {
    ...
    setPendingEncryptedFile(null); // Clear any pending locked files
    setPdfPassword("");
  ...
  // MODIFIED: Intercept the file to check for password protection
  const processFile = async (file: File, password?: string) => {
    if (file.type === "application/pdf") {
      ...
      try {
        ...
        // This will throw if it's encrypted and no password/wrong password is provided
        const pdfDoc = await PDFDocument.load(arrayBuffer, { password });
        
        let finalFileToUpload = file;

        // If we successfully unlocked it with a password, save it stripped of encryption
        if (password) {
          const unencryptedBytes = await pdfDoc.save();
          finalFileToUpload = new File([unencryptedBytes], file.name, { type: "application/pdf" });
        }

        // Proceed normally with the clean file
        uploadStatement(finalFileToUpload);
        setStatementName(file.name);
        sessionStorage.setItem("anti_ai_active_statement_name", file.name);
        setShowErrors(false);
        setPdfPreviewUrl(URL.createObjectURL(finalFileToUpload));
        ...
        if (msg.toLowerCase().includes("encrypted") || msg.toLowerCase().includes("password")) {
          ...
          if (password) setPdfAuthError("Incorrect password. Please try again.");
        } else {
          toast({ title: "PDF Error", description: "Could not read PDF structure.", variant: "destructive" });
          console.error(err);
        }
      } finally {
      ...
      // It's a CSV or other file, bypass decryption logic
  ...
  return (
    <Layout title="Import Bank Statement" subtitle="Human-in-the-Loop Statement Parsing Workspace">
      <div className="flex flex-col gap-5 max-w-[1800px] mx-auto relative">
        ...
        {/* MODIFIED: Drag and Drop OR Password Prompt */}
        {stagedTransactions.length === 0 ? (
          pendingEncryptedFile ? (
            <div className="mt-6 border-2 border-slate-200 rounded-2xl p-10 flex flex-col items-center justify-center text-center bg-white shadow-sm max-w-lg mx-auto">
              <div className="w-14 h-14 rounded-2xl bg-amber-50 border border-amber-100 flex items-center justify-center text-xl mb-4 text-amber-600">🔒</div>
              <h3 className="text-base font-bold text-slate-800">Password Protected Document</h3>
              <p className="text-slate-500 mt-1.5 mb-6 text-xs leading-relaxed max-w-sm">
                {pendingEncryptedFile.name} requires a password to unlock. The decryption happens safely on your device.
              </p>
              ...
              <div className="w-full max-w-xs space-y-3">
                <input 
                  ...
                  onKeyDown={e => e.key === 'Enter' && pdfPassword && processFile(pendingEncryptedFile, pdfPassword)}
                  placeholder="Enter PDF password..." 
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 rounded-lg text-sm outline-none transition-all"
                  autoFocus
                />
                
                {pdfAuthError && (
                  <p className="text-xs font-medium text-rose-600 bg-rose-50 p-2 rounded-md">{pdfAuthError}</p>
                )}
                ...
                <div className="flex gap-2 w-full pt-2">
                  ...
                  <button 
                    onClick={() => processFile(pendingEncryptedFile, pdfPassword)} 
                    disabled={!pdfPassword || isDecrypting} 
                    className="flex-1 px-4 py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-sm font-semibold transition disabled:opacity-50"
                  >
                    {isDecrypting ? "Unlocking..." : "Unlock & Import"}
                  </button>
                </div>
              </div>
            </div>
            ...
            >
              ...
              <label className="px-5 py-2.5 bg-slate-900 text-white rounded-lg cursor-pointer hover:bg-slate-800 transition-all text-xs font-semibold shadow-md">
                ...
                <input type="file" className="hidden" onChange={(e) => e.target.files && processFile(e.target.files[0])} disabled={isUploading} accept=".pdf,.csv" />
              </label>
            </div>
          )
        ) : (
              ...
              {pdfPreviewUrl ? (
                ...
                <div className="flex flex-col items-center justify-center text-center p-6 bg-slate-50 rounded-xl border border-dashed border-slate-300 h-[600px] sticky top-6 shadow-sm">
                  ...
                  <label className="mt-4 px-4 py-1.5 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 rounded-md cursor-pointer transition text-xs font-semibold shadow-sm">
                    Re-link PDF
                    <input type="file" className="hidden" accept=".pdf" onChange={(e) => { if (e.target.files?.[0]?.type === "application/pdf") { processFile(e.target.files[0]); } }} />
                  </label>
                </div>
              )}
            </div>
