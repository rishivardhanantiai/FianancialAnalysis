...
const NOISE_PATTERNS: RegExp[] = [
  /^PAGE\s+NO/i, /^ACCOUNT\s+BRANCH/i, /^ACCOUNT\s+STATUS/i, /^A\/C\s+OPEN\s+DATE/i,
  /^STATEMENT\s+OF\s+ACCOUNT/i, /^STATEMENT\s+SUMMARY/i, /^OPENING\s+BALANCE/i,
  /^CLOSING\s+BAL/i, /^CLOSING\s+BALANCE\s+INCLUDES/i, /CONTENTS\s+OF\s+THIS\s+STATEMENT/i,
  /^REGISTERED\s+OFFICE/i, /^HDFC\s+BANK\s+LIMITED/i, /FUNDS\s+EARMARKED/i,
  /STATE\s+ACCOUNT\s+BRANCH\s+GSTN/i, /GSTIN\s+NUMBER/i, /HTTPS?:\/\//i,
  /^GENERATED\s+ON/i, /^GENERATED\s+BY/i, /^REQUESTING\s+BRANCH/i,
  /COMPUTER\s+GENERATED\s+STATEMENT/i, /NOT\s+REQUIRE\s+SIGNATURE/i,
  /\bDATE\b.*\bNARRATION\b/i, /\bTXN\s+DATE\b.*\bBALANCE\b/i, /\bVALUE\s+DATE\b.*\bBALANCE\b/i,
  /^ACCOUNT\s+NAME/i, /^DRAWING\s+POWER/i, /^INTEREST\s+RATE/i, /^MOD\s+BALANCE/i,
  /^CIF\s+NO/i, /^IFS\s+CODE/i, /^MICR\s+CODE/i, /^NOMINATION\s+REGISTERED/i,
  /^BALANCE\s+AS\s+ON/i, /^ACCOUNT\s+STATEMENT\s+FROM/i, /^\bADDRESS\b/i,
  /^\bACCOUNT\s+NUMBER\b/i, /^\bACCOUNT\s+DESCRIPTION\b/i
];

// ============================================================================
// 3. UTILITY FUNCTIONS (Math, Strings, Dates)
// ============================================================================

function patchPdf2JsonWarnings(): void {
  const globalWithFlag = globalThis as typeof globalThis & { __bankParserWarnPatched?: boolean };
...
function toIsoDate(day: number, month: number, rawYear: string): string | null {
  ...
  if (
    ...
    day < 1 || month < 1 || month > 12 || year < 1990 || year > 2099
  ) return null;
...
function parseAmountText(raw: string): number | null {
  ...
  return Number.isFinite(value) ? Math.abs(Number(value.toFixed(2))) : null;
}
...
function sanitizePartyName(value: string): string {
  return value
    .replace(/[@].*$/g, "")
    .replace(/\b\d{6,}\b/g, "")
...
function extractCustomer(notes: string): string {
  ...
  const chunks = cleaned.split(/\s+-\s+|\/+/).map(normalizeWhitespace).filter(Boolean);
...
function normalizeFinalNotes(notes: string): string {
  return cleanDescription(notes) || "Bank transaction";
}

// ============================================================================
// 4. PDF GEOMETRY & PARSING HELPERS
// ============================================================================
...
function itemsInRange(row: TextRow, range: Range): TextItem[] {
  return row.items.filter((item) => item.x >= range.min && item.x <= range.max);
}

function itemsInRightEdgeRange(row: TextRow, range: Range): TextItem[] {
...
function joinNarrationItems(items: TextItem[]): string {
...
function getColumnAmount(row: TextRow, range: Range): number | null {
  return parseAmountText(rowText(itemsInRange(row, range)));
}

function getHdfcColumnAmount(row: TextRow, range: Range): number | null {
...
function groupItemsIntoRows(items: TextItem[], tolerance: number): TextRow[] {
  ...
  for (const page of pages) {
    ...
    for (const item of pageItems) {
      ...
      rows.push({ items: sorted, page, y: currentY, text: rowText(sorted) });
      current = [item];
      currentY = item.y;
    }
...
function amountLikeGroups(row: TextRow): { x: number; value: number }[] {
  ...
  const push = () => {
    ...
    if (value !== null && !isDate) groups.push({ x: current[0].x, value });
    current = [];
  };
...
function startsLikelyContinuation(row: TextRow, narrationRange: Range): boolean {
...
// ============================================================================
// 5. BANK ENGINES (Logic Identical, Structure Improved)
// ============================================================================
...
function parseHdfcRows(rows: TextRow[]): ParsedDraft[] {
  ...
  for (const row of rows) {
    ...
    if (rowDate) {
      flush();
      currentTx = { date: rowDate, amount: 0, type: "Expense", notes: "", _w: null, _d: null, _b: null };
    }
...
function parseGenericLedgerRows(rows: TextRow[]): ParsedDraft[] {
  ...
  for (const row of rows) {
    ...
    if (debit !== null && credit === null) {
      ...
      else if (credit > 0 && debit === 0) { amount = credit; type = "Revenue"; }
    }
...
// ============================================================================
// 6. ENGINE REGISTRY & SUMMARY CHECKS
// ============================================================================
...
const PARSER_REGISTRY: BankExtractionEngine[] = [
  ...
  {
    name: "Generic Bank",
    matchesSignature: () => true, // Fallback
...
function determineParsingEngine(documentText: string): BankExtractionEngine {
  return PARSER_REGISTRY.find((engine) => engine.matchesSignature(documentText)) ?? PARSER_REGISTRY[3];
}

// ============================================================================
// 7. CORE PARSER ENTRY POINTS
// ============================================================================
...
function extractPdfRows(fileBuffer: Buffer): Promise<TextRow[]> {
  return new Promise((resolve, reject) => {
    ...
    parser.on("pdfParser_dataError", (errData: any) => {
      reject(new Error(`Failed to parse PDF data: ${errData?.parserError || "Unknown error"}`));
    });
    ...
    // CRITICAL FIX: Ensure synchronous errors thrown during parseBuffer reject the promise immediately.
    try {
